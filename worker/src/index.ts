...
- Do NOT include a calendar_add/follow_up action when: the message is past-tense or about something already finished (e.g. "the meeting went well yesterday", "thanks for calling earlier"); it's a vague or rhetorical expression with no real intent to schedule or act ("we should really catch up sometime", "I keep meaning to call them"); or the task/event clearly belongs to someone else in the conversation, not the user (e.g. one participant reminding another participant, in a group, about that other person's own commitment). These can read like a real proposal on the surface but are not one — omit action entirely rather than guessing.
- Examples (message → action decision): "Let's grab dinner Friday at 7" → calendar_add, confident:true. "We should really catch up sometime" → no action (rhetorical, no real plan). "Can you send me the invoice by tomorrow?" → follow_up, confident:true. "The meeting went really well yesterday" → no action (past tense, already happened). "Someone remind Sarah to bring her laptop" → no action, since Sarah is the one with the task, not the user.`;
...
function buildPrompt(body: SuggestRequest, intents: string[]): string {
  ...
  const earlierContextBlock = earlierContext && earlierContext.length > 0
    ? `<earlier_context>\n${earlierContext.map((m) => `${m.sender ?? 'Me'}: ${m.text}`).join('\n')}\n</earlier_context>\nThe above is background from just before this exchange — already addressed, not something to reply to again. Still consider it, alongside the current message, when identifying a calendar_add/follow_up action: the proposal or task itself may have been stated there rather than in the current message.\n\n`
    : '';
