...
- **Investigation started 2026-07-30 into an actual fix (not another symptom patch).** `Notification.getShortcutId()` (API 30+) is a promising lead not yet tried — unlike `sbn.id` (an app-chosen int, confirmed reused across unrelated conversations on the test device), a shortcutId is a system-managed, semantically per-conversation identifier that backs Android's own Conversations/Bubbles features, so it's much less likely to collide the same way. Not yet trusted enough to switch the actual key on, though — this project has an established history of OEM (OPPO/ColorOS) notification-framework quirks (`NoSuchMethodError` on `areBubblesEnabled()`, channel state not sticking — see `project-bubble-debug` memory), so a blind swap risks repeating the 2026-06-16 regression for a different reason. Shipped instead: privacy-safe diagnostic logging (`logShortcutIdDiagnostic()` in `ProTxtBgService.kt`, hashes only, never raw title/name, unconditional so it's visible in the release build real usage happens on, not just debug builds) that flags via `Log.w("ProTxtDiag", ...)` whenever the same convKey is observed with two different shortcutId hashes over time — i.e., a real, evidenced collision candidate. Next step is gathering real logcat evidence (`adb logcat -s ProTxtDiag`) before deciding whether to actually build the shortcutId-based fix.
- **UI symptom of the collision, mitigated 2026-07-30:** a contact who has two technically-distinct identities under the same display name (per the collision above) got offered as a link target for *itself* in Contacts → contact detail → "+ Link another app" — the picker's `getUnmatchedSenders()` list has no concept of "this is the contact I'm already viewing," so the same-named orphaned identity showed up as a normal linkable sender (repro'd live with "Tom Test," the same test contact used throughout this issue's history). `ContactDetailModal.tsx`'s `filteredUnmatched` now excludes any sender whose display name matches the open contact's own name. This is a UI-level guard, not a fix for the underlying convKey collision — the two identities are still separate under the hood, just no longer offered as a self-link.
...
- **Per-contact data insights** — 2026-07-30. Reply-time, weekly volume trend, most active hour, and most common message intent surfaced in Contacts → contact detail's new "Insights" section. Reuses `ContactSignals.kt`'s existing per-platform rolling signal data (no new tracking) rolled up across every linked platform, plus a `style_edits.intent` aggregate for "usually messages about." Reply-time trending ("taking longer to reply") deferred — no historical snapshot exists yet to compare against.
...
**Also reported 2026-08-06 — show an "overdue" state before the auto-clear removes it.** Rather than a follow-up (or an Upcoming calendar/booking item — same "time has passed" shape) silently vanishing once auto-clear fires, indicate visually that it's past its official time during the grace-buffer window: e.g. a distinct "Overdue" badge/muted style in `FollowUpsScreen.tsx` (and the Upcoming screen, if extended there too) the moment `dueAt`/the event's `date` has passed, kept up until the item is actually removed or marked done. Gives the user a chance to notice and act on something before it disappears on its own, instead of the auto-clear feeling like data loss.

### Handle international/cross-timezone times correctly
Reported 2026-08-06, prompted by the "leave" nudge work (see BUGS.md BUG-003/004/005) but a broader gap. All date/time handling today is deliberately naive wall-clock: the client sends its own local time with no zone suffix (`worker/src/index.ts`'s `formatNow`/`localDateTime`, explicitly "no zone" per its own comment), and the worker never does real timezone math — by design, since without a zone it's safer to omit time-of-day than state a wrong one. That's fine while everything mentioned is in the user's own timezone, but breaks down for international travel: a flight departing 14:00 in New York and landing 18:00 in London are on different UTC offsets, and naively diffing two local-time strings (for a leave-time calculation, an "am I going to make it" check, or just displaying "Today"/"Tomorrow" in `upcomingEvents.ts`'s `dayStart`, which uses the device's own zone throughout) will misread the actual gap once a trip crosses zones — worse across a DST boundary. Needs a real design pass: where do we get each leg's actual zone (departure vs. arrival airport, not just device zone), and which of the naive-local-time call sites (worker prompt, `dayStart`/`formatEventSubtitle`/`formatTravelSubtitle` in `upcomingEvents.ts`, any future leave-nudge calculation) actually need to become zone-aware vs. can stay as-is. Not yet scoped.
