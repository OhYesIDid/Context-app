...
object DeviceContactsResolver {
    ...
    /** Normalizes to the last 10 digits, for comparing phone numbers across formatting/country-code differences. */
    fun normalizePhone(raw: String): String? {
        val digits = raw.filter { it.isDigit() }
        return if (digits.length >= 7) digits.takeLast(10) else null
    }

    /** Returns normalized phone numbers for the device contact whose primary display name exactly matches [name]. */
    fun displayNameToPhones(context: Context, name: String): List<String> = try {
        context.contentResolver.query(
            ...
            while (cursor.moveToNext()) {
                cursor.getString(numCol)?.let { normalizePhone(it) }?.let(result::add)
            }
            result
        } ?: emptyList()
